...
- **Remote environment management & remote execution** — a parent environment centrally manages its children: add by URL, parallel status checks (version snapshot / inference state / online), one-click incremental push updates (push model: children need not reach the parent; the parent can be anonymous or intranet-only); bind any chat session to a child for "remote execution" — inference stays on the parent while all tools run in the child, with terminal, workspace files and file journal operable straight from the parent UI; two transports — direct HTTP and a WS reverse tunnel (the child dials out, penetrating NAT/LAN) — so even children without a public address can be managed and executed remotely. [Read more](docs/introduce-remote-execution.md)
