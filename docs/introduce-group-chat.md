...
- **@ 驱动 + 连续对话路由**：消息里点名谁谁响应；不点名时默认由上一轮最后响应的 Agent 接着回答。若群聊尚无任何 Agent 回复，则优先交给带 `leader` 标签的参与者（可多个），没有 leader 才广播全组；
...
| ② | **@mention / @all 定向唤起与连续路由** | `@昵称`/`@AgentID` 点名唤醒；`@all`（或 `@everyone`）广播全体；不 @ 时延续上一位响应者，新群聊则优先路由给带 `leader` 标签的参与者，无 leader 才广播全组 |
...
- **不 @ 直接发送**：若已有 Agent 回复，默认交给上一轮最后响应的 Agent；若尚无任何 Agent 回复，则交给所有带 `leader` 标签的参与者；一个 leader 都没有时才广播给所有参与者。
...
3. **输入**：键入 `@` 唤起成员菜单，`@某成员` 定向点名；若不 @ 直接发送，则默认与上一位响应者继续对话。新群聊优先由 `leader` 响应，无 leader 才广播全员。
...
| 环节 | 触发方式 | 并发/产物 | 关键点 |
|---|---|---|---|
| **Round 1** | 用户消息的 @ 与上下文路由 | 有 @ 只唤点名成员；无 @ 时延续最后响应者，新群聊则 leader 优先、无 leader 广播全体 | 实际路由到的 Agent ID 被记录到用户消息的 `mentions` |
