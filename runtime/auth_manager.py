...
class AuthManager:
    ...
    def create_setup_token(self) -> tuple[str, str]:
        ...
        Setup tokens are only consumed by this same server, so a compact
        binary format is used (the first implementation used a self-contained
        JSON payload plus a full HMAC-SHA256 signature, which was visually
        noisy in one-line install commands and is no longer accepted):
    ...
    def verify_setup_token(self, token: str) -> bool:
        with self._lock:
            if not self._enabled or self._fail_closed:
                return False
            return self._verify_compact_setup_token(token, self._setup_secret)
