...
- **多任务并发对话及实时状态跟踪** — 支持多个聊天会话同时进行，每个会话独立管理流式状态；通过 SSE 实时更新会话状态（流式中、成功、错误、未读）；基于用户滚动位置自动管理已读状态；会话标题实时广播更新
- **多浏览器共享会话实时推理** — 同一会话可在多个浏览器或标签页中同步观看和操作；服务端保留并广播当前用户消息、assistant/token、thinking、工具调用及结果等推理帧，新加入的浏览器可重放尚未落盘的实时进度；切换会话或浏览器后可继续接收推理，重新发送末次推理（Continue）时对旧 assistant 消息的移除也会同步到其他浏览器
- **飞行模式（Flight Mode）** — 可按会话开启后台持续推理；开启后即使所有浏览器都断开连接、关闭页面或切走，推理任务仍会在服务端继续执行并持久化，稍后重新打开即可恢复观看；未开启飞行模式时，最后一个浏览器离开后会取消无人观察的推理，避免无意消耗模型资源
...
- **远程环境管理与远程执行** — 母端集中管理子环境：URL 添加、并行状态检查（版本快照/推理状态/在线）、一键推送增量更新（推送模型：子端无需能访问母端，母端可匿名或仅内网）；聊天会话可绑定子环境「远程执行」——推理留在母端，全部工具在子端执行，终端、工作区文件与文件 journal 均可在母端 UI 直接操作；支持直连 HTTP 与 WS 反向隧道两种传输，子端主动拨号母端、穿透 NAT/内网，无公网地址的子端也能被集中管理与远程执行。详见 [远程环境管理与远程执行](docs/introduce-remote-execution.md)
...
> **安全说明（远程环境隧道）：** 子→母反向隧道端点（`/v1/tunnel/*`、`/v1/tunnel-proxy/*`）与上述端点同样位于 `/v1/` 鉴权门之后：母服务设置密码时，子端使用其 `SETUP_SOURCE` 安装链接中的 token 拨号鉴权；母服务未设置密码（鉴权关闭）时，这些端点完全开放——任何能到达母端口的客户端都可注册隧道并把请求转发到子端本地端点（含执行子端工具）。使用远程环境时，请勿在未设置密码的情况下将母端端口暴露到不受信任的网络。
